...
double *y;
float default_theta = (30 * PI) / 180.f;
...
int main(int argc, char **argv) {
    ...
    while (!WindowShouldClose()) {
        ...
        double theta2dot = (-g / l) * sinf(theta) -
                           (tao)*theta_dot * cosf(theta) * cosf(theta);
        ...
        snprintf(current, 50, "Induced current: %2.1fA",
                 -(b * l * w * theta_dot * cosf(theta)) / R);
        snprintf(k, 50, "Kinetic energy of system: %2.1fJ",
                 m * l * theta_dot / 2);
...
#define CREATE_SLIDER(name, x, y, min, max, val)                            \
    float f##val = val;                                                     \
    GuiSlider((Rectangle){x, y, 100, 20}, name, TextFormat("(%f)", f##val), \
              &f##val, min, max);                                           \
    val = f##val;

        CREATE_SLIDER("B-field", 100, 100, 0.0f, 10.0f, b);
        CREATE_SLIDER("Mass", 100, 120, 1.0f, 100.f, m);
        CREATE_SLIDER("Length", 100, 140, 0.1f, 1.0f, l);
        CREATE_SLIDER("Loop depth", 100, 160, 0.1f, 1.0f, d);

        CREATE_SLIDER("Initial angle", 100, 180, -PI, PI, default_theta);

        if (GuiButton((Rectangle){10, 220, 100, 20}, "Restart")) {
            theta = default_theta;
            theta_dot = 0;
        }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            DrawLineV((Vector2){(i - half) * width + w / 2, 0},
                      (Vector2){(i - half) * width + w / 2, h},
                      (Color){0, 0, 100, 50});
            DrawTriangleLines(
                mid,
                (Vector2){(i - half) * width + w / 2 - width / 2 + 2,
                          h / 2 + width},
                (Vector2){(i - half) * width + w / 2 + width / 2 - 2,
                          h / 2 + width},
                (Color){0, 0, 100, 50});
